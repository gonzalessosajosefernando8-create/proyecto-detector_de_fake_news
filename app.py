...
def procesar_consulta_dinamica(url_ingresada, texto_investigacion, pregunta_usuario):
    ...
    # Filtrado semántico: Limpiamos conectores del motor de búsqueda de Google
    ...
    try:
        ...
        print(f"🔍 Buscando evidencias en tiempo real para: {query_final}")
        ...
        for idx, link in enumerate(enlaces):
            ...
            if len(fuentes_vivas) < 2:
                fuentes_vivas.append({
                    ...
                    "snippet": f"Portal indexado útil para contrastar de manera empírica las afirmaciones de la investigación."
                })
    except Exception as e:
        print(f"⚠️ Error en Google Search: {e}")

    # Sistema de contingencia estable (Solo si falla Google Search)
    if not fuentes_vivas:
        fuentes_vivas.append({
            "url": "https://alicia.concytec.gob.pe/",
            "titulo": "Repositorio Nacional Digital ALICIA - CONCYTEC",
            "snippet": "Catálogo nacional oficial para verificar tesis y artículos científicos indexados por el Estado peruano."
        })

    # Construcción dinámica del enlace hacia Google Académico
    fuentes_sugeridas.append({
        ...
        "snippet": "Filtro especializado en documentos arbitrados por pares académicos ideales para respaldar marcos teóricos."
    })
...
def index():
    ...
    if request.method == "POST":
        # 📥 ACCIÓN: Guardar comentarios de forma directa en Supabase
        if request.form.get("accion") == "feedback":
            comentario = request.form.get("comentario_texto") or ""

            if comentario.strip():
                try:
                    # Insertar el registro de forma directa en tu tabla de Supabase
                    data, count = supabase.table("sugerencias").insert({
